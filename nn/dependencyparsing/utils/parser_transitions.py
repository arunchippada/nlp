...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "LA":
            x = self.stack.pop(-2)
            self.dependencies.append((self.stack[-1], x))
            return self
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        partial_parse_predictions = zip(minibatch, predictions)
        parses_next = [pp.parse_step(t) for (pp, t) in partial_parse_predictions]
        new_unfinished_parses = [pn for pn in parses_next if pn.buffer or len(pn.stack) > 1]
        unfinished_parses.extend(new_unfinished_parses)
