...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        nn.init.xavier_uniform_(w)
        self.hidden_to_logits_weight = nn.Parameter(w)

        b = torch.empty(self.n_classes)
        nn.init.uniform_(b)
        self.hidden_to_logits_bias = nn.Parameter(b)
    ...
    def embedding_lookup(self, w):
        ...
        selected_embeddings = self.embeddings.index_select(0, w.flatten())
        return selected_embeddings.view(w.shape[0], -1)
    ...
    def forward(self, w):
        ...
        h = self.relu(x.matmul(self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        d = self.dropout(h)
        logits = d.matmul(self.hidden_to_logits_weight) + self.hidden_to_logits_bias
        return logits
